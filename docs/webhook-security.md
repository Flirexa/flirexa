...
```bash
# All webhook traffic
journalctl -u vpnmanager-api -n 500 | grep -iE 'webhook|complete_payment|signature'

# Per-provider
journalctl -u vpnmanager-api -n 500 | grep -iE 'nowpayments|cryptopay|paypal|stripe|razorpay|mollie|payme'

# Recovery poller activity
journalctl -u vpnmanager-api -n 500 | grep -iE 'recover.*payment|stuck.*pending'

# Subscriptions activated in last hour
journalctl -u vpnmanager-api --since "1 hour ago" | grep -iE 'subscription.*upgraded|subscription.*activated'
```
