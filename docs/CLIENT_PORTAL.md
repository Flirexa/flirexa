...
async def serve_frontend(request, full_path):
    if full_path == "download/app":
        # Serves latest versioned APK (Flirexa-v*.apk), fallback to vpnmanager-vpn.apk
        versioned = sorted(glob("Flirexa-v*.apk"), reverse=True)
...
```json
{
  ...
  "release_notes": "Flirexa VPN — ocean-themed WireGuard client"
}
```
...
Then deploy: `cp -r client-portal-dist/ /opt/vpnmanager/src/web/`
