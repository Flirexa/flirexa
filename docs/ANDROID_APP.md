# ANDROID_APP — Flirexa VPN Android Application  @ANDROID_APP
...
| APK on server | `/opt/vpnmanager/src/web/static/Flirexa-v1.1.0.apk` |
| APK metadata | `/opt/vpnmanager/src/web/static/apk-version.json` |
...
```
android-app/
├── ui/src/main/java/com/vpnmanager/vpn/
│   ├── activity/
...
└── ui/src/main/res/
    ...
    ├── values/
    │   ├── styles.xml             # AppThemeBase (fontFamily=fredoka_bold), FlirexaNavIndicator
    │   ├── themes.xml             # WireGuardTheme extensions
    │   ├── colors.xml             # Full palette + vpnmanager_card_bg_alpha (#CCF5D76E)
...
3. Copy: cp android-app/ui/build/outputs/apk/release/*.apk src/web/static/Flirexa-vX.Y.Z.apk
4. Update src/web/static/apk-version.json (version, version_code, filename, date, changelog)
5. Deploy to production (scp + restart vpnmanager-client-portal.service)
...
cp ui/build/outputs/apk/release/*.apk ../src/web/static/Flirexa-v1.1.0.apk
```
