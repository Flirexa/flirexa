<!-- Technical multi-location controls retained for operators who explicitly
     select the Advanced browser experience. Native apps use the same backend
     regardless of which browser component is rendered. -->
<template>
  <div class="fx-page">
        ...
        <div class="fx-slot-server-grid">
          <button v-for="srv in slot.servers"
            ...
            <div class="fx-slot-server-name">
              <CountryFlag
                :code="srv.country_code"
                :location="srv.server_location"
                :name="srv.server_display_name || srv.server_name"
                :size="28"
              />
              <span class="fx-slot-server-copy">
                <span class="fx-slot-server-title">{{ srv.server_display_name || srv.server_name }}</span>
                <span class="fx-slot-server-protocol">{{ protocolLabel(srv.server_type) }}</span>
              </span>
              <span v-if="fastestServerId === srv.server_id && pingResults[srv.server_id]?.rtt_ms != null"
                    class="fx-badge fx-badge-accent fx-slot-fastest">
                {{ $t('devices.fastest') }}
              </span>
            </div>
...
}[String(type || '').toLowerCase()] || String(type || 'WireGuard'))
...
.fx-slot-server-ping.fast {
  background: var(--bg-elev);
  border: 1px solid color-mix(in oklab, var(--success) 36%, var(--border));
  color: var(--success);
}
...
.fx-slot-server-copy { flex:1;min-width:0;display:flex;flex-direction:column;gap:2px; }
.fx-slot-server-title { display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12.5px;font-weight:650; }
.fx-slot-server-protocol { display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;color:var(--text-3); }
.fx-slot-fastest { flex-shrink:0;font-size:9.5px;height:20px; }
.fx-slot-server-meta {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
