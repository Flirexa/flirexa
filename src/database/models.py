...
class ClientShareToken(Base):
    """One-time-ish, time-limited token authorising a public download of a
    client's WireGuard config. Generated by the operator from the Clients
    page when they need to hand a fresh config to a customer without sending
    the .conf as an attachment.

    Rows are short-lived (default 10 min) and idempotently cleaned up on
    access — no separate worker needed. The `used_at` column records when
    the customer downloaded the file; the row itself is kept for a short
    audit window so the operator can confirm "yes, they actually downloaded".
    """
    ...
    # IP that downloaded — best-effort audit. Useful when the customer says
    # "the link didn't work" so the operator can see whether anyone actually
    # hit it.
    download_ip: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
