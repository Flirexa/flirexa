...
class BackupManager:
        ...
        if is_proxy:
            ...
            try:
                if server_type == "hysteria2":
                    ...
                    validator = VlessRealityManager(
                        config_path=config_path or "/etc/xray/config.json",
                        service_name=getattr(server, "proxy_service_name", None) or "xray-reality",
                        listen_port=getattr(server, "port", 443) or 443,
                        domain=getattr(server, "proxy_domain", None),
                        reality_private_key=getattr(server, "proxy_reality_private_key", None),
                        reality_public_key=getattr(server, "proxy_reality_public_key", None),
                        short_id=getattr(server, "proxy_reality_short_id", None),
                        ssh_host=server.ssh_host,
                        ssh_port=server.ssh_port or 22,
                        ssh_user=server.ssh_user or "root",
                        ssh_password=server.ssh_password,
    ...
    def _backup_proxy_config(self, server: Server, backup_dir: str):
        ...
        config_path = getattr(server, "proxy_config_path", None) or (
            ...
            else "/etc/xray/config.json" if server_type == "vless-reality"
    ...
    def _managed_services(self) -> list[str]:
        ...
        legacy = [
            ...
            for suffix in ("api", "worker", "admin-bot", "client-bot", "client-portal")
        ]
