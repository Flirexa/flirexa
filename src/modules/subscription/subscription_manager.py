"""
Flirexa Client Portal - ClientPortalSubscription Manager
...
def is_free_tier_enabled(db: Session) -> bool:
    """Whether new users get an auto-created free subscription.

    Persisted in ``system_config`` (key ``enable_free_tier``) so admins can
    turn the free tier off and force every visitor through a paid plan.
    Default ``True`` keeps existing installs working unchanged.
    """
    try:
        from src.database.models import SystemConfig
        row = db.query(SystemConfig).filter(
            SystemConfig.key == "enable_free_tier"
        ).first()
    except Exception:
        return True
...
class SubscriptionManager:
    ...
    def create_user(
        ...
        ``email`` is optional since migration 039 — pass None (or empty
        string, which gets normalised) for username-only registration.
        Account recovery is impossible without an email on file; the
        UI surfaces a warning when the customer leaves the field
        blank, see Register.vue.

        Returns:
            (user, error_message)
        """
        # Normalise blank/whitespace email to None so the unique index
        # doesn't reject the second username-only signup as a duplicate
        # empty-string row.
        email = (email or "").strip() or None

        # Validate email + username uniqueness — return a unified message
        # for either collision so an attacker can't enumerate registered
        # email addresses or usernames by submitting candidates and
        # reading the error string. Both branches still log the actual
        # cause for ops/debug.
        if email and self.db.query(ClientUser).filter(ClientUser.email == email).first():
            logger.info("register: email collision for %s", email)
        ...
        if self.db.query(ClientUser).filter(ClientUser.username == username).first():
            logger.info("register: username collision for %s", username)
            return None, "An account with this email or username already exists"

        # Hash password
        password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

        # Verification token is only useful when there's an email to send
        # the verification link to. Skip when email is None.
        ...
        # and skip entirely if admin has turned the free tier off).
        if is_free_tier_enabled(self.db):
            try:
        ...
        logger.info(f"Created user: {username} ({email or 'no email'})")
        return user, None

    def authenticate_user(self, email: str, password: str) -> Optional[ClientUser]:
        """Authenticate by email only (legacy entrypoint — prefer
        ``authenticate_user_by_identifier`` which accepts either)."""
        return self.authenticate_user_by_identifier(email, password)
    ...
    def authenticate_user_by_identifier(
        ...
        Migration 039 made email optional; customers that signed up
        with username only literally can't log in with the old
        email-only auth path. We pick the lookup column by whether the
        identifier looks like an email — if it has an ``@`` we treat
        it as an email, otherwise as a username. Username lookups
        succeed even when the user has an email on file, so this is
        strictly additive.
        """
        if not identifier:
            return None
        ...
        if "@" in identifier:
            user = self.db.query(ClientUser).filter(
                ClientUser.email == identifier
            ).first()
            # The '@' heuristic locks out usernames that CONTAIN an '@'
            # (e.g. a customer who registered as "@erza"): the email lookup
            # misses and the username branch never runs. Fall back to the
            # username column — strictly additive, still password-checked.
            if not user:
                user = self.db.query(ClientUser).filter(
                    ClientUser.username == identifier
                ).first()
        else:
            user = self.db.query(ClientUser).filter(
                ClientUser.username == identifier
    ...
    def ensure_subscription(self, user_id: int) -> Optional[ClientPortalSubscription]:
        ...
        When the admin has disabled the free tier (``system_config.enable_free_tier``
        false), returns the existing row if any but never auto-creates a free
        subscription — the portal then renders a "Choose a plan" CTA instead
        of silently letting the user past the device limit on free defaults.
        """
        sub = self.get_subscription(user_id)
        if sub:
            return sub
        if not is_free_tier_enabled(self.db):
            return None
    ...
    def complete_payment(self, invoice_id: str, tx_hash: Optional[str] = None, sync_wg: bool = True) -> bool:
        ...
        #
        # ensure_subscription returns None when the user has no prior
        # subscription AND the free tier is disabled in system_config
        # (e.g. operator wants to push paid plans only). In that case
        # the original logic just dropped through — the payment was
        # marked completed but no subscription row ever got created,
        # leaving a successful payment without its entitlement.
        #
        # Now: if we get here with no subscription and the payment
        # carries a non-free tier, create the paid subscription
        # directly from the payment's tier + duration. Skip the
        # upgrade_subscription call below — create_subscription has
        # already set the right expiry, and upgrade_subscription would
        # stack `duration_days` on top of that, double-counting.
        subscription = self.ensure_subscription(payment.user_id)
        just_created_paid = False
        if (
            not subscription
            and payment.subscription_tier
            and payment.subscription_tier.strip().lower() != "free"
        ):
            try:
                ...
                just_created_paid = True
                logger.info(
                    "[PAY:%s] Created paid subscription %s for user %d "
                    "(free tier disabled, no prior sub) — %d days",
                    invoice_id, payment.subscription_tier, payment.user_id,
                    payment.duration_days or 30,
                )
                if tracer:
                    tracer.step("create_paid_sub", status="ok", detail={
                        "tier": payment.subscription_tier,
                        "days": payment.duration_days or 30,
                    })
            except Exception as _sub_err:
                logger.error(
                    "[PAY:%s] Failed to create paid subscription %s for user %d: %s",
                    invoice_id, payment.subscription_tier, payment.user_id, _sub_err,
                )
                if tracer:
        ...
        #
        # The old code also auto-created a WG client for users with
        # zero peers ("auto_create_wireguard_client"). That step was
        # removed per operator request (2026-05-26): a single auto-
        # peer collides with the portal's Add Device flow on multi-
        # region setups, the customer hits the device cap before they
        # can pick a region themselves. Customers now click Add
        # Device on the portal after their first payment to mint
        # configs the right way. apply_subscription_limits still runs
        # so any existing peers get the new tier's bandwidth /
        # traffic / expiry applied.
        ...
        try:
            ...
            if user and sub:
                ...
                # Email receipt — best-effort. SMTP not configured / user
                # has no email / template failure all silently no-op.
                try:
                    import os as _os
                    smtp_enabled = _os.getenv("SMTP_ENABLED", "false").lower() == "true"
                    smtp_host = _os.getenv("SMTP_HOST", "").strip()
                    if smtp_enabled and smtp_host and user.email:
                        ...
                        EmailService(
                            ...
                            currency="USD",
                            period_days=payment.duration_days or 30,
                            new_expiry_date=expiry_str,
                    ...
                    logger.debug(f"Payment-received email failed (non-critical): {_email_err}")
        except Exception as e:
    ...
    def get_payment_by_invoice(self, invoice_id: str, for_update: bool = False) -> Optional[ClientPortalPayment]:
        """
        Get payment by invoice ID.

        Set ``for_update=True`` from webhook handlers to acquire a row-level
        lock so concurrent webhook deliveries for the same payment serialise.
        The status check after the lookup then reflects committed state, not
        stale snapshot. ``complete_payment`` itself also takes the lock as a
        ...
        if for_update:
            try:
                ...
                # Fallback for DBs that don't support FOR UPDATE (SQLite tests).
                pass
        return q.first()
