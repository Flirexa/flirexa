...
"""
    ...
    def bootstrap(
        ...
        # See note in hysteria2.py — same UFW issue applies to TUIC.
        try:
            ...
            if ufw_rc == 0:
                ...
                if status_rc == 0 and "Status: active" in (status_out or ""):
                    ...
                    if rule not in (status_out or ""):
                        ...
                        self._run(f"ufw allow {rule} comment 'tuic ({self.service_name})' 2>&1")
                        details["firewall_opened"] = rule
        except Exception as _fw_err:
            logger.warning(f"Best-effort UFW rule for tuic failed: {_fw_err}")
