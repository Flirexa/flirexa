...
class ServerManager:
    ...
    def update_server(
        ...
        **kwargs
        ...
        allowed_fields = {
            ...
            "ipv4_only", "is_active", "customer_visible",
            "customer_visible_mobile", "customer_visible_windows",
            "for_app_only", "force_visible",
        ...
        try:
            ...
            if self._is_proxy(server):
                ...
                try:
                    ...
                    if force:
                        logger.warning(
                            f"_get_wg failed for {server.name}: {e}. force=True — skipping remote cleanup entirely, DB delete proceeds."
                        )
                    else:
                        logger.error(f"Cannot delete server: _get_wg failed: {e}")
                        return False
    ...
    def generate_server_config(self, server_id: int) -> Optional[str]:
        ...
        # security: derive the subnet from a parsed IPv4 network object instead
        # of raw string-slicing, and shlex.quote() the subnet + interface before
        # they land in the root PostUp/PostDown shell (executed by wg-quick).
        # For a valid pool like 10.66.0.0/19 and interface wg0 both quote to the
        # exact same token, so existing configs stay byte-identical.
        import ipaddress as _ipaddr
        _pool = server.address_pool_ipv4 if "/" in server.address_pool_ipv4 else f"{server.address_pool_ipv4}/24"
        wg_subnet = str(_ipaddr.ip_network(_pool, strict=False))
        _subnet_q = shlex.quote(wg_subnet)
        _iface_q = shlex.quote(server.interface)
        config = (
            f"[Interface]\n"
            f"Address = {address}\n"
            ...
            f"iptables -C INPUT -p udp --dport {server.listen_port} -j ACCEPT 2>/dev/null || "
            f"iptables -I INPUT 1 -p udp --dport {server.listen_port} -j ACCEPT; "
            f"iptables -t nat -A POSTROUTING -s {_subnet_q} -o $ETH -j MASQUERADE; "
            ...
            f"iptables -D INPUT -p udp --dport {server.listen_port} -j ACCEPT 2>/dev/null || true; "
            f"iptables -t nat -D POSTROUTING -s {_subnet_q} -o $ETH -j MASQUERADE; "
