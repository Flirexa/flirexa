...
class Hysteria2Manager(ProxyBaseManager):
    ...
    def bootstrap(
        ...
        # 4. Open the listening port in the host firewall (best-effort).
        # Servers added via "Install Proxy" on top of an existing WG host
        # often have UFW active with a DROP policy and no rule for the
        # proxy port, so the daemon listens but packets are silently dropped.
        try:
            ufw_rc, _, _ = self._run("which ufw 2>/dev/null")
            if ufw_rc == 0:
                status_rc, status_out, _ = self._run("ufw status 2>/dev/null")
                if status_rc == 0 and "Status: active" in (status_out or ""):
                    rule = f"{self.listen_port}/udp"
                    if rule not in (status_out or ""):
                        _log(f"🛡  UFW active — opening {rule}")
                        self._run(f"ufw allow {rule} comment 'hysteria2 ({self.service_name})' 2>&1")
                        details["firewall_opened"] = rule
        except Exception as _fw_err:
            logger.warning(f"Best-effort UFW rule for hysteria2 failed: {_fw_err}")
