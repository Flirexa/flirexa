...
# Server-level circuit breaker: when get_all_peers() raises, suspend
# traffic sync for that server for `_UNREACHABLE_BACKOFF` seconds so the
# worker doesn't log "Failed to sync traffic for X" once per client per
# minute against a dead box. Any successful re-fetch clears the entry.
_SERVER_UNREACHABLE_UNTIL: Dict[int, datetime] = {}
_UNREACHABLE_BACKOFF = 600  # 10 minutes
...
class TrafficManager:
    ...
    def _get_cached_peer_transfer(self, server: Server, public_key: str) -> tuple:
        ...
        Server-level circuit breaker: if a previous fetch raised, the
        server is marked unreachable for `_UNREACHABLE_BACKOFF` seconds
        and subsequent calls short-circuit to (0, 0) without retrying.
        A successful fetch clears the entry. This stops the worker from
        logging "Failed to sync traffic" once per client per cycle when a
        remote box has gone offline."""
        ...
        with _TRAFFIC_CACHE_LOCK:
            # Circuit breaker: skip immediately if server is in backoff.
            unreachable_until = _SERVER_UNREACHABLE_UNTIL.get(server_id)
            if unreachable_until and now < unreachable_until:
                return _TRAFFIC_STATS_CACHE.get(server_id, {}).get(public_key)
            ...
            if server_id in _TRAFFIC_STATS_CACHE and server_id in _TRAFFIC_STATS_CACHE_TIME:
                ...
                if cache_age < _TRAFFIC_STATS_CACHE_TTL:
                    return _TRAFFIC_STATS_CACHE[server_id].get(public_key)
        ...
        try:
            ...
            # Mark server unreachable for _UNREACHABLE_BACKOFF seconds.
            # ONE warning per backoff window (suppressed inside the
            # window above), then we go silent until the breaker opens
            # again on the next attempt.
            with _TRAFFIC_CACHE_LOCK:
                _SERVER_UNREACHABLE_UNTIL[server_id] = now + timedelta(seconds=_UNREACHABLE_BACKOFF)
            logger.warning(
                f"Traffic-sync circuit breaker tripped for server {server.name} (id={server_id}): "
                f"{type(e).__name__}: {e}. Suspending sync for {_UNREACHABLE_BACKOFF}s."
            )
            # Return stale cache if any, else zeros — so the per-client loop
            # doesn't error every iteration. Real traffic counters resume
            # once the server heals.
            return _TRAFFIC_STATS_CACHE.get(server_id, {}).get(public_key)
        ...
        with _TRAFFIC_CACHE_LOCK:
            ...
            # Successful fetch heals the breaker.
            _SERVER_UNREACHABLE_UNTIL.pop(server_id, None)

        return peer_map.get(public_key)
    ...
    def get_client_traffic(self, client: Client) -> TrafficStats:
        ...
        if server:
            ...
            _cur = self._get_cached_peer_transfer(server, client.public_key)
            if _cur is None:
                # Peer not on the live interface this read (pre-handshake / iface just
                # restarted / partial list). Don't treat as 0 — report stored traffic
                # unchanged by reading at the current baseline (delta = 0).
                current_rx, current_tx = client.traffic_baseline_rx, client.traffic_baseline_tx
            else:
                current_rx, current_tx = _cur
        else:
    ...
    def reset_traffic_counter(self, client_id: int) -> bool:
        ...
        if server:
            _cur = self._get_cached_peer_transfer(server, client.public_key)
            # Peer not visible right now → treat current counters as 0 (a not-yet-
            # connected peer's real counter is ~0 anyway). used is being zeroed below.
            current_rx, current_tx = _cur if _cur is not None else (0, 0)
        else:
            current_rx, current_tx = self.wg_manager.get_peer_transfer(client.public_key)
        ...
        for client in clients:
            try:
                ...
                if server:
                    _cur = self._get_cached_peer_transfer(server, client.public_key)
                    if _cur is None:
                        # Peer absent from the live interface this tick (pre-handshake,
                        # iface just restarted, partial agent list, or server in backoff
                        # with no cached value). Skip — do NOT rebase the baseline to a
                        # spurious 0, which would double-count the peer's whole counter
                        # when it reappears next tick.
                        continue
                    current_rx, current_tx = _cur
                else:
                    current_rx, current_tx = self.wg_manager.get_peer_transfer(client.public_key)
