...
class ManagementCore:
    ...
    def get_system_status(self) -> Dict:
        ...
        # ONE aggregate over the enabled clients — collapses the separate
        # active-count, the six expiry COUNT queries (formerly
        # timers.get_expiry_summary), and the traffic sums into a single grouped
        # table scan. At tens of thousands of clients this turns ~8 sequential
        # scans of the client table into one. Reads only integer/date columns,
        # never the four Fernet-encrypted ones, so cost stays flat. Semantics are
        # kept byte-identical to the old per-query logic (same `now`, same
        # bucket boundaries; verified old==new on the live DB before shipping).
        from sqlalchemy import func as _sfunc, case as _scase, cast as _scast, BigInteger as _sBigInt
        from datetime import datetime as _dt, timezone as _tz, timedelta as _td
        _now = _dt.now(_tz.utc)
        _used = _sfunc.coalesce(Client.traffic_used_rx, 0) + _sfunc.coalesce(Client.traffic_used_tx, 0)

        def _sc(cond):
            # SUM(CASE WHEN cond THEN 1 ELSE 0) — a counted bucket, NULL-safe.
            return _sfunc.coalesce(_sfunc.sum(_scase((cond, 1), else_=0)), 0)

        _agg = (
            self.db.query(
                _sfunc.count(Client.id),
                _sfunc.coalesce(_sfunc.sum(_sfunc.coalesce(Client.traffic_used_rx, 0)), 0),
                _sfunc.coalesce(_sfunc.sum(_sfunc.coalesce(Client.traffic_used_tx, 0)), 0),
                _sc(
                    (Client.traffic_limit_mb.isnot(None))
                    & (Client.traffic_limit_mb > 0)
                    & (_used > _scast(Client.traffic_limit_mb, _sBigInt) * 1024 * 1024)
                ),
                _sc(Client.expiry_date.is_(None)),                                             # no_expiry
                _sc(Client.expiry_date <= _now),                                               # expired
                _sc((Client.expiry_date > _now) & (Client.expiry_date <= _now + _td(days=1))),  # today
                _sc((Client.expiry_date > _now) & (Client.expiry_date <= _now + _td(days=7))),  # week
                _sc((Client.expiry_date > _now) & (Client.expiry_date <= _now + _td(days=30))), # month
            )
            .filter(Client.enabled == True)
            .one()
        )
        enabled_count = int(_agg[0] or 0)
        total_rx = int(_agg[1] or 0)
        total_tx = int(_agg[2] or 0)
        exceeded_count = int(_agg[3] or 0)
        active_clients = enabled_count
        expiry_summary = {
            "total_clients": enabled_count,
            "no_expiry": int(_agg[4] or 0),
            "expired": int(_agg[5] or 0),
            "expiring_today": int(_agg[6] or 0),
            "expiring_week": int(_agg[7] or 0),
            "expiring_month": int(_agg[8] or 0),
        }
